require "teeplate"
require "file_utils"
require "ecr"

module AzuCLI
  module Generators
    # Base class for all generators implementing the Template Method pattern
    # This class defines the skeleton of the generation algorithm
    abstract class Base < Teeplate::FileTree
      getter output_dir : String
      getter name : String
      getter generate_specs : Bool

      def initialize(@name : String, @output_dir : String = "src", @generate_specs : Bool = true)
        validate_name!
      end

      # Template method defining the generation algorithm
      def generate!
        validate_preconditions!
        prepare_output_directory!
        output_path = build_output_path
        render_template

        # Generate spec file if enabled and template exists
        if @generate_specs
          generate_spec_file!
        end

        post_generation_hook
        output_path
      end

      # Abstract methods to be implemented by subclasses
      abstract def build_output_path : String
      abstract def template_directory : String

      # Optional methods for spec generation
      protected def spec_template_name : String
        "#{@name}_spec.cr.ecr"
      end

      protected def build_spec_output_path : String
        File.join("spec", spec_directory, "#{@name}_spec.cr")
      end

      protected def spec_directory : String
        # Default spec directory structure mirrors src structure
        relative_path = build_output_path.gsub(@output_dir, "").lstrip("/")
        File.dirname(relative_path)
      end

      # Hook methods that can be overridden by subclasses
      protected def validate_preconditions!
        # Default implementation - can be overridden
      end

      protected def prepare_output_directory!
        output_path = build_output_path
        FileUtils.mkdir_p(File.dirname(output_path))
      end

      protected def render_template
        # Use Teeplate's render method to generate files
        render(File.dirname(build_output_path))
      end

      protected def generate_spec_file!
        spec_output_path = build_spec_output_path
        spec_template_path = File.join(template_directory, spec_template_name)

        if File.exists?(spec_template_path)
          FileUtils.mkdir_p(File.dirname(spec_output_path))
          render_spec_template(spec_output_path, spec_template_path)
        end
      end

      # Default implementation creates a basic spec file
      # Subclasses can override this for custom spec generation
      protected def render_spec_template(output_path : String, template_path : String)
        spec_content = generate_basic_spec_content
        File.write(output_path, spec_content)
      end

      protected def generate_basic_spec_content : String
        <<-SPEC
        require "spec"
        require "../spec_helper"
        require "../../src/#{spec_directory}/#{@name}"

        describe #{name_camelcase} do
          describe "structure" do
            it "can be instantiated" do
              # Add your tests here
              true.should be_true
            end
          end
        end
        SPEC
      end

      protected def post_generation_hook
        # Default implementation - can be overridden
      end

      protected def validate_name!
        raise ArgumentError.new("Name cannot be empty") if @name.empty?
        raise ArgumentError.new("Name must be a valid identifier") unless valid_identifier?(@name)
      end

      # Utility methods for common naming conventions
      protected def name_camelcase
        @name.camelcase
      end

      protected def name_snakecase
        @name.underscore
      end

      protected def name_pluralized
        # Simple pluralization - can be enhanced with a proper pluralization library
        @name.ends_with?("s") ? @name : @name + "s"
      end

      protected def name_kebabcase
        @name.gsub(/[A-Z]/, "-\\0").downcase.lstrip("-")
      end

      private def valid_identifier?(name : String) : Bool
        # Check if name is a valid Crystal identifier
        name.matches?(/^[a-zA-Z_][a-zA-Z0-9_]*$/)
      end
    end
  end
end
